...
if __name__ == '__main__':
    ...
    lstm_layers = 4
    input_shape = (2, temporal_frames, height, width)
    print("INPUT_SHAPE {}".format(input_shape))
    output_shape = (2, output_time_horizon, 32, 32)
    dtype = torch.float
    time_steps = (
            input_time_window - temporal_frames + 1
    )
    encoder = E3DLSTM(
    ...
    #print(encoder)
    decoder = nn.Conv3d(
        hidden_size * time_steps, output_shape[0], kernel, padding=(0, 2, 2)
    ).type(dtype)
    #params = parameters(recurse=True)
